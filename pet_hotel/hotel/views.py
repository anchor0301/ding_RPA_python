...
def agreement_view(request, token):
    ...
    # 서명한 고객이 다시 접근할 경우도 404로 막을 수 있음
    if customer.agreement_signed:
        return redirect(reverse('hotel:reserve') + f'?token={token}')
...
def create_agreement_link(customer):
    return f"https://yourdomain.com/agreement/{customer.token}/"
