...
# 새 연락처 생성 (vCard 형식)
def create_vcard(owner_name, full_name, phone_number):
    ...
    vcard.n.value = vobject.vcard.Name(family=full_name, given=owner_name)
...
# CardDAV 서버에 연락처 추가
def add_contact_to_carddav(owner_name, full_name, phone_number):
    vcard_data = create_vcard(owner_name, full_name, phone_number)
    # 고유한 파일명 (UUID 사용)
